...
DECLARE v_id_rol INTEGER;
v_telefono TEXT;
v_nombre TEXT;
v_especialidad INTEGER;
v_matricula TEXT;
v_fecha_nac TEXT;
BEGIN -- Extrae los metadatos enviados desde Flutter en `data: {...}`
...
IF v_id_rol = 4 THEN
INSERT INTO public.paciente (id_paciente, fecha_nacimiento)
VALUES (
    NEW.id,
...
-- ------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.handle_cita_status_change() RETURNS TRIGGER AS $$ BEGIN IF OLD.estado IS DISTINCT
...
AFTER
UPDATE ON public.cita FOR EACH ROW EXECUTE FUNCTION public.handle_cita_status_change();
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.obtener_historial_citas() RETURNS TABLE (
